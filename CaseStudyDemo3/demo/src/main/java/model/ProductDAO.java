...
//    }
    ...
    public Object
    public HashMap<String,Integer> selectOrderDetail()throws SQLException{
        HashMap<String,Integer> hashMap = new HashMap<>();
        try (Connection connection = getConnection();
             PreparedStatement statement = connection.prepareStatement(SELECT_ORDER_DETAIL_SQL);){
            updateTypePrice(); // cập nhật column typePrice trong orderdetail
            ResultSet rs = statement.executeQuery();
            ...
            while (rs.next()){
                ...
                int typePrice = rs.getInt("typePrice");
                hashMap.put(productId,amount);
            }
        }catch (SQLException e){
            e.printStackTrace();
        }
